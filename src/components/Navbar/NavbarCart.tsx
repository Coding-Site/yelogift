          ...
          carts.map((cart, idx) => (
            <div className="flex justify-start gap-3 w-full " key={idx}>
              ...
              <div className="flex basis-24 text-base h-8 min-w-[100px] px-3 items-center ms-auto w-auto  justify-between rounded-full border border-gray-300">
                <span onClick={() => dispatch(decreaseOneItem(cart.product?.id as number))} className="cursor-pointer">
                  -
                </span>
                <span>{cart.quantity}</span>
                <span onClick={() => dispatch(increaseOneItem(cart.product?.id as number))}  className="cursor-pointer">+</span>
              </div>
            </div>
          ))
        ) : (
