    ...
    <div className="flex flex-col gap-6">
      <div className="flex justify-start sm:justify-center gap-x-3 w-full container overflow-x-auto scrollbar-none">
        ...
        >
          ...
          <span
            className={`${
              'all' == active ? "bg-main text-mainBlack" : "text-main"
        ...
        {cats.map((item) => (
          ...
          >
            ...
            <span
              className={`${
                item.name.toLocaleLowerCase() == active ? "bg-main text-mainBlack" : "text-main"
              } py-1 px-2 rounded-full text-xs`}
            >
              {item.name}
            </span>
      ...
      <div className="felx flex-col">
        ...
        if(active == "all" || active == cat.name.toLocaleLowerCase()) return (        
          <CardsFeed key={idx} title={cat.name} link={`categories/${cat.id}`} products={cat.products as IProduct[]} />
