...
function EditPart() {
  ...
  useEffect(() => {
    setLoading(true)
    instance.get(`/api/admin/product/parts/${partId}`, {
      headers: {
  ...
  const onSubmit: SubmitHandler<Inputs> = (data: any) => {
    ...
    instance
      .post(`/api/admin/product/parts/update`, { ...data, product_id: productId, part_id: partId }, {
        headers: {
          Authorization: `Bearer ${adminToken}`,
        },
      })
      .then(() => { setLoading(false); navigate(`/admin/products/${productId}/parts`) })
              ...
              <div className="flex flex-col gap-2 relative">
                ...
                <label htmlFor="auto" className="flex items-center gap-x-2">
                  ...
                  <input
                    ...
                    type="radio"
                    defaultChecked={watch("selling_type") == "auto"}
                    id="auto"
                    value="auto"
                ...
                <label htmlFor="manual" className="flex items-center gap-x-2">
                  ...
                  <input
                    {...register("selling_type")}
                    type="radio"
                    defaultChecked={watch("selling_type") == "manual"}
                    value="manual"
                    id="manual"
                    className="border hidden border-gray-400 rounded-md bg-transparent p-1"
                  />
